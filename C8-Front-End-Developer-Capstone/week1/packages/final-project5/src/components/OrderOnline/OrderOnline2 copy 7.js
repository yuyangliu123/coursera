import { Box, Button, HStack, Heading, Image, Skeleton, Stack, Text, VStack, getToastPlacement } from "@chakra-ui/react";
...
const OrderOnline2 = () => {
  ...
  //set scroller height
  const stackMinHeight = ((menu.data.length === 0 ? 20 : menu.data.length) % itemInfo.eachColCount) === 0
    ? Math.floor((menu.data.length === 0 ? 20 : menu.data.length) / itemInfo.eachColCount) * (itemInfo.itemHeight) + Math.floor((menu.data.length === 0 ? 20 : menu.data.length) / itemInfo.eachColCount - 1) * (itemInfo.marginBottom + borderWidth * 2) + "px"
    : Math.floor((menu.data.length === 0 ? 20 : menu.data.length) / itemInfo.eachColCount + 1) * (itemInfo.itemHeight) + Math.floor((menu.data.length === 0 ? 20 : menu.data.length) / itemInfo.eachColCount) * (itemInfo.marginBottom + borderWidth * 2) + "px";
  console.log(stackMinHeight);
  ...
  if (Object.keys(menu.data).length > 0 || loading) {
    return (
      <VStack>
