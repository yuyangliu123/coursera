...
const OrderOnline2 = () => {
  ...
  const elements = useMemo(() => {
    ...
    if (dataToDisplay.length > 0) {
      return dataToDisplay.map(value => (
  ...
  //set scroller height
  const stackMinHeight = Array.isArray(searchResults.data) && searchResults.data.length > 0
    ? (searchResults.data.length % itemInfo.eachColCount) === 0
      ? Math.floor(searchResults.data.length / itemInfo.eachColCount) * (itemInfo.itemHeight) + Math.floor(searchResults.data.length / itemInfo.eachColCount - 1) * (itemInfo.marginBottom + borderWidth * 2) + "px"
      : Math.floor(searchResults.data.length / itemInfo.eachColCount + 1) * (itemInfo.itemHeight) + Math.floor(searchResults.data.length / itemInfo.eachColCount) * (itemInfo.marginBottom + borderWidth * 2) + "px"
  ...
  if (Object.keys(menu.data).length > 0 || searchResults) {
    return (
      <VStack>
