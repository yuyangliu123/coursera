...
const OrderOnline2 = () => {
  ...
  const getKey = (pageIndex, previousPageData) => {
    ...
    const finalUrl = sort ? `${baseUrl}&sort=${sort}` : baseUrl;
    return finalUrl;
  };
  ...
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      console.log("Intersection callback triggered", entries[0]);
  ...
  console.log("elements", elements, "datatodisplay", dataToDisplay, "searchResults", searchResults, "pages", pages, isLoading,
    "isLoading", "isSearching", isSearching, "infiniteScroll", infiniteScroll, "showSkeleton", showSkeleton);
                ...
                }
                {
                  ...
                  <Box backgroundColor={itemInfo.backgroundColor}>
                    <WindowElement
                      ...
                      eachColCount={itemInfo.eachColCount}
                    />
                    {/* {infiniteScroll && <Box id="sentinel" ref={sentinelRef} style={{ height: '1px' }}>} */}
                      {showSkeleton &&
