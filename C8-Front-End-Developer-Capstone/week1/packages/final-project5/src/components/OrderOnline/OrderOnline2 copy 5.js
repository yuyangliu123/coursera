import { Box, Button, HStack, Heading, Image, Spinner, Stack, Text, VStack, Skeleton, SkeletonText, useToast, getToastPlacement } from "@chakra-ui/react";import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
...
const OrderOnline2 = () => {
  ...
  //infinite scroll
  useEffect(() => {
    console.log("useEffect triggered", location, page, window.location.href); // 添加日志来确认 useEffect 是否被触发
    ...
    fetchData();
  }, [location, page, window.location.href
  ]);
  ...
  const handleCategoryClick = (category) => {
    ...
    newUrl =
      !currentCategory
        ...
        :
        category != currentCategory ? `/order2?category=${category}` : '/order2';
    setPage(1);
  ...
  const SkeletonCategories = () => (
    <Box width={{ lg: "20%", base: "100%" }} backgroundColor="#fbdabb4d" height="fit-content" padding="0 0 3vh 3vh">
      <Box textStyle="StyledNav" fontSize="2em">
        Ingredient
      </Box>
      <VStack marginLeft="0" width="fit-content" alignItems="self-start">
        {Array.from({ length: 5 }).map((_, index) => (
          <Skeleton height="40px" key={index} width="100%" />
        ))}
      </VStack>
    </Box>
  );

  const SkeletonWindowElement = () => (
    <Stack width={{ lg: "70%", base: "100%" }}>
      ...
      <Box marginTop="2rem">
        {Array.from({ length: 6 }).map((_, index) => (
          <Skeleton key={index} height="400px" width="100%" marginBottom="30px" />
        ))}
      </Box>
    </Stack>
  );
