...
const BookingForm = () => {
  ...
  const checkReservation = useCallback(async (date) => {
    ...
    const response = await fetch(`http://localhost:5000/reservation/checkReservation?resDate=${date}`);
    const data = await response.json();
    setResAvailable(data);
  }, []);
          ...
          <VStack spacing={4} align="flex-start">
            ...
            <FormControl>
              <Input type={inputType} name="resDate" {...register('resDate')} placeholder='Choose Date'
                onFocus={handleFocus} onBlur={(e) => { handleBlur(); checkReservation(e.target.value); }} />
