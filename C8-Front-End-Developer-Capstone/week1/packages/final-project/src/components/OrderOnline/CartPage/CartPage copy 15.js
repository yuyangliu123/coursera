...
const CartPage = ({ initialData, initialInputValue, loading, error, fetchlike, imageWidth }) => {
    ...
    const instantHandleCrtState = async ({ items, isChecked, value }) => {
        try {
            ...
            try {
                const endpoint = "http://localhost:5000/shoppingcart/updateCart";

                const payload = { updatedItems: itemsToUpdate };
                const result = await (await axiosInstanceWithTokenCheck()).post(endpoint, payload)
                if (result.status === 200) {
                    // setCartData(prevCartData => ({
                    //     ...prevCartData,
    ...
    //     if (isEmail && location.pathname !== '/cart') {
    //         // 呼叫 mergeCart
    //         // 注意：這裡的 mergeCart() 可能需要從某個地方獲取 token，或者您需要重新設計它
    //         console.log("leave ./cart");

    //         mergeCart({ token: someToken });
    //     }

    // }, [location.pathname, isEmail]);
    // 在 CartPage 組件內部
    // useEffect(() => {
    //     // 這裡可以檢查使用者是否已登入，以及購物車中是否有待合併的項目
                                    ...
                                    <Td colSpan={2} gridArea={"meal"} display={{ md: "table-cell", base: "flex" }} flexDirection="row" borderBottomWidth={{ md: "1px", base: "0" }} alignItems="center" paddingLeft="0" paddingRight="0">
                                        ...
                                        <Button
                                            ...
                                            onClick={async () => {
                                                ...
                                                await instantHandleCrtState({ items: item, isChecked: item.checked, value: 0 })
