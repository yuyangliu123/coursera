...
const CartPage = () => {
    ...
    //     );
    // };
    const handleCheckState = async (itemsToUpdate, isChecked) => {
        try {
            if (isEmail) {
                // 已登录用户：直接将check state发送到Redis
                let result = await (await axiosInstanceWithTokenCheck()).post(
                    "http://localhost:5000/api/updateCheckState",
                    {
                        updatedItems: itemsToUpdate,
                ...
                // 处理响应 - 本地状态已经通过乐观更新修改，这里主要处理错误
                if (!result || result.data.status !== "ok") {
                    console.error("Failed to update check state in Redis");
                    // 可以添加错误处理或回滚本地状态的逻辑
                }
            } else {
                // 未登录用户：仍然使用IndexedDB
