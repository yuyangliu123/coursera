    ...
    const handleCheckState = async (items, isChecked) => {
        try {
            ...
            setPendingItems(prev => {
                // Convert items to array if it isn't already
                // const itemsArray = Array.isArray(items) ? items : [items];

                // Filter out items that are already in prev
                // const newItems = itemsToUpdate.filter(item =>
                //     !prev.some(prevItem =>
                //         prevItem.idMeal === item.idMeal
                //     )
                // );
                const newItems = itemsToUpdate
                console.log("setPendingItems complete");
            ...
            console.log(pendingItems, "outsidependingItems");

            if (isEmail) {
                console.log(pendingItems, "userpendingItems");


                debouncedSendCheckStateToServer();
            } else {
                // 未登录用户：仍然使用IndexedDB
                const db = await initDB();
                let existingCart = await db.get('cart', 'cartData');
    ...
    const debouncedSendCheckStateToServer = useCallback(
        debounceRAF(async () => {
            try {
                ...
                const latestPendingItems = pendingItemsRef.current;
                console.log("Sending to server: latestPendingItems", latestPendingItems);

                const result = await (await axiosInstanceWithTokenCheck()).post(
                    "http://localhost:5000/api/updateCheckState",
                    { pendingItems: latestPendingItems }
                );
                if (result.status === 200) {
                    setCartData(prevCartData => ({
                        ...prevCartData,
