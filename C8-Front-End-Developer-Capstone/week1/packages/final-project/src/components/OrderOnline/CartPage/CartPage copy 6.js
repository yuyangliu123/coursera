...
const CartPage = ({ initialData, loading, error, fetchlike, imageWidth }) => {
    ...
    const handleCheckState = async (items, isChecked) => {
        try {
            ...
            if (isEmail) {
                debouncedSendCheckStateToServer(itemsToUpdate, isChecked);
            } else {
                // 未登录用户：仍然使用IndexedDB
                const db = await initDB();
                let existingCart = await db.get('cart', 'cartData');
    ...
    const debouncedSendCheckStateToServer = useCallback(
        debounceRAF(async (itemsArray, isChecked) => {
            try {
                const result = await (await axiosInstanceWithTokenCheck()).post(
                    "http://localhost:5000/api/updateCheckState",
                    { updatedItems: itemsArray, isChecked }
                );
                if (result.status === 200) {
                    setCartData(prevCartData => ({
                        ...prevCartData,
