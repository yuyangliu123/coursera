...
const FoodPage2 = () => {
    ...
    const { email, isEmail, accessToken } = useUserRotate();
    ...
    const handleLike = async () => {
        ...
        try {
            let result = await apiClient.post("http://localhost:5000/api/update",
                {
                    event: "like",
    ...
    const onOrder = async () => {
        ...
        if (!isEmail) {
            ...
            try {
                let result = await apiClient.post("http://localhost:5000/api/addToCart", orderData);
                if (result.status === 200) {
                    setCartItem(currentCartItem => currentCartItem + numMeal);
                }
            } catch (err) {
