...
const MiniCart2=({cart,showMiniCart,setShowMiniCart,ref})=>{
  //if !cart(empty the shopping cart)
  if (!cart) {
    return (
      <Box display={showMiniCart ? "block" : "none"} backgroundColor="#fff" right="0" minWidth="200px" maxHeight="500px" border="1px solid #ccc">
        No Item In Cart
      </Box>
    );
  }
    return(
        ...
        >
          <VStack>
          <Box alignSelf="start" margin="1rem 0 0 1rem"  textStyle="StyledNav">Your Cart ({cart.totalItem} items)</Box>
          <Button
          onClick={() => setShowMiniCart(!showMiniCart)}
          ...
          >
            ...
            <HashLink to="/cart"><Button sx={theme.textStyles.StyledButton.baseStyle} size="md">View Cart</Button></HashLink>
              <Box alignSelf="end" marginRight="1rem" textStyle="StyledNav">Total Price (${cart.totalAmount})</Box>
            </Box>
          </VStack>
        </Box>
    )
